...
| `repo.ts`   | One scratch git repo under `$HOME`, registered with `factory status` (or fully installed with `--install`), ready for `mission`, `step`, `gate` and `decision`. Every CLI assertion starts here. Exports `makeRepo`, `run`. | 2026-09-07 |
...
| `spawn.ts`  | The one real spawn, one model run: a scratch project with the Factory installed, a mission, and a headless session told to run one `@Worker` whose handoff carries a LOW decision. Prints the `additionalContext` the parent received and the mission's `decisions.md`. | 2026-09-07 |
...
REAL_HOME=$HOME HOME=$(mktemp -d /private/tmp/vspawn.XXXXXX) bun .factory/validator/spawn.ts
```
...
and `CLAUDE_CONFIG_DIR` all read `Not logged in`. `spawn.ts` therefore runs the session with the
real `HOME` and prefixes the project's own hook commands with `HOME=<scratch>`, so every Factory
