...
| Preset    | Spawn-time bundle for a session: prompt file, settings overlay, MCP list, model, effort, plugins.         |
...
| Gate      | A step that blocks until a decision. `human` gates are drawn in the session by the plugin `gate` tool and mirrored in the Inbox, answered in either. `orchestrator` gates are recorded decisions with a reason. `factory step` refuses to pass a gate unanswered. |
...
| Question     | A sub-agent request for human input, raised through the `ask` tool, drawn in the session and mirrored in the Inbox, answered in either. Fallback: blocked return, orchestrator asks. |
...
| handoffs/     | Sub-agents   | End of each step    | Final-message handoff saved by hook: done, undone, commands with exit codes, issues, deviations, faster ways. Loose rule, may be skipped with a reason. |
