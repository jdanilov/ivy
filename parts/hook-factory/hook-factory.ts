 ...
 *
 * Appends one line to ~/.factory/events/<session>.jsonl, keeps the Mac awake while a
 * mission-bound session is mid-turn, injects mission focus before a compaction, and saves
 * a sub-agent's final message as the step handoff.
 *
...
async function readMission(dir: string): Promise<Bound | null> {
  ...
  return { dir, name, title: state.title ?? name, step: state.step ?? '', round: state.round ?? 0, gates: state.gates ?? {} };
}
...
async function bind(cwd: string, session: string): Promise<Bound | null> {
  ...
  if (!claim || claim.session !== session) return null;
  ...
  for (const entry of await readdir(missions).catch(() => [])) {
    ...
    if (found?.name === claim.mission) return found;
  }
  return null;
}
