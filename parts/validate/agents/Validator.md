...
tools: Read, Glob, Grep, Bash, Skill, mcp__factory__ask
...
is the app reachable. It fails, or there is no such recipe: call `mcp__factory__ask` with what you
need (a URL, a command, a login) and three concrete options. Without the tool, stop and return
blocked with that question. Never guess a port and never launch a server behind the human's back.
